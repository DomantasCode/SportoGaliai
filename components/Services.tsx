...
const Services: React.FC = () => {
  ...
  return (
    <section id="services" className="py-24 md:py-32 bg-dark relative overflow-hidden border-y border-white/5">
      {/* Background Pattern */}
      ...
      <div className="container mx-auto px-4 md:px-6 2xl:px-12 3xl:px-6 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-6">
          ...
          <p className="text-gray-400 w-full md:max-w-md pb-2 text-base md:text-lg md:text-right">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {SERVICES.map((service, index) => {
            ...
            return (
              <div
                key={index}
                className="group relative bg-dark p-8 rounded-[2rem] border-2 border-primary/30 hover:border-secondary hover:bg-dark/90 transition-all duration-500 overflow-hidden cursor-pointer shadow-lg hover:shadow-2xl hover:shadow-secondary/30 flex flex-col justify-between h-[380px]"
                onClick={() => setSelectedService(service)}
              >
                {/* Watermark Icon on Hover */}
                <div className="absolute -right-8 -bottom-8 text-white opacity-0 group-hover:opacity-5 transition-opacity duration-500 transform group-hover:scale-110 pointer-events-none">
                ...
                <div>
                   <div className="flex justify-between items-start mb-8">
                      <div className="w-14 h-14 bg-white/10 rounded-2xl flex items-center justify-center text-secondary group-hover:bg-white/20 group-hover:text-secondary transition-colors duration-300">
                   ...
                   <h4 className="text-2xl font-bold text-white mb-3 group-hover:text-secondary transition-colors duration-300 line-clamp-2">
                       {service.title}
                   </h4>
                   <p className="text-white/70 text-sm leading-relaxed group-hover:text-white/90 transition-colors duration-300 line-clamp-4">
                     {service.description}
                   </p>
                </div>

                {/* Bottom Part */}
                <div className="pt-6 mt-auto border-t border-white/10 group-hover:border-white/20 transition-colors duration-300">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-bold text-secondary group-hover:text-white transition-colors duration-300 uppercase tracking-wider">
                      Plačiau
                    </span>
                    <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-white group-hover:bg-secondary group-hover:text-primary transition-all duration-300 group-hover:translate-x-1">
      ...
      {selectedService && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="bg-white w-full max-w-3xl rounded-[2.5rem] overflow-hidden shadow-2xl relative z-10 animate-in zoom-in-95 slide-in-from-bottom-4 duration-300 max-h-[90vh] flex flex-col">
             {/* Close Button */}
             <button 
              onClick={() => setSelectedService(null)}
            ...
            <div className="p-6 md:p-8 overflow-y-auto">
               <div className="flex flex-col md:flex-row gap-6 md:gap-8">
                 <div className="flex-1">
                   <h4 className="text-primary font-bold text-xs uppercase tracking-wider mb-3 border-b border-gray-100 pb-2">Apie paslaugą</h4>
                 ...
                 {selectedService.features && (
                   <div className="md:w-1/3 shrink-0">
                      <h4 className="text-primary font-bold text-xs uppercase tracking-wider mb-3 border-b border-gray-100 pb-2">Ką gausite?</h4>
